...
void SmartPacketRadio::initialize()
{
    ...
    address=par("address");
    if(not address) 0; //TODO find any /dev/radio*

    char device[20];
    sprintf(device,"/dev/radio%u",address);
    fd=open(device,O_RDWR);
    if(fd<0) throw cRuntimeError(this,"Couldn't open radio %d.",address);

    tcflush(fd,TCIFLUSH);
    tcsetattr(fd,TCSANOW,&t);

    char reset=0x01;
    char setAddress[]={0x08,address};
    char mac[]={0x02,0x00}; // for setting the MAC algorithm.
    if(write(fd,&reset,1)<0 or
       write(fd,setAddress,sizeof(setAddress))<0 or
       write(fd,mac,sizeof(mac))<0)
