...
void Sink::handleMessage(cMessage *msg)
{
    ...
    if (tokenizer.hasMoreTokens()) source = atoi(tokenizer.nextToken());
    if (tokenizer.hasMoreTokens()) dest = atoi(tokenizer.nextToken());
    if (tokenizer.hasMoreTokens()) packetNumber = atoi(tokenizer.nextToken());
    if (tokenizer.hasMoreTokens()) packetType = *tokenizer.nextToken();

    // Prints each incoming packet
    EV<< "Source: "<<source<<" dest: "<<dest<<" #"<<packetNumber;
    if (packetType == 'a') EV<<" (ack)";
    if (packetType == 'h') EV<<" (hello)";
    EV<<"\n";
