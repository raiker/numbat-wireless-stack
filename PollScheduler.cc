...
PollScheduler::PollScheduler() : cScheduler(),
n_fds(0)
{
    for(int i=0;i<sizeof(poll_fd)/sizeof(pollfd);i++)
    {
        poll_fd[i].fd=-1;
...
{
    for(int i=0;i<n_fds;i++) delete message[i];
}
...
cMessage *PollScheduler::getNextEvent()
{
    ...
    if(msg and msg->getArrivalTime()==simTime()) return msg;
    ...
    if(n_fds>0) poll(poll_fd,n_fds,timeout);
    else if(msg) usleep(timeout*1000);
    else throw cTerminationException(eENDEDOK);

    for(int i=0;i<n_fds;i++) if(poll_fd[i].revents&POLLIN)
    {
        poll_fd[i].revents=0;
        gettimeofday(&now,NULL);
        now=now-start;
